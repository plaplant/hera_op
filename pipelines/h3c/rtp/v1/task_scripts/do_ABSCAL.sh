...
# make calfits and omni_vis file names
omni_fn=`echo ${fn%.uvh5}.omni.calfits`
omni_vis_fn=`echo ${fn%.uvh5}.omni_vis.uvh5`

# call omni-abscal script; see hera_cal.abscal for more details
echo post_redcal_abscal_run.py ${omni_vis_fn} ${omni_fn} ${model_files} --raw_auto_file ${fn} --data_is_redsol --model_is_redundant \
         --nInt_to_load ${nInt_to_load} --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut}  --phs_max_iter ${phs_max_iter} \
         --phs_conv_crit ${phs_conv_crit} --edge_cut ${edge_cut} --clobber --verbose
post_redcal_abscal_run.py ${omni_vis_fn} ${omni_fn} ${model_files} --raw_auto_file ${fn} --data_is_redsol --model_is_redundant \
    --nInt_to_load ${nInt_to_load} --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut}  --phs_max_iter ${phs_max_iter} \
    --phs_conv_crit ${phs_conv_crit} --edge_cut ${edge_cut} --clobber --verbose
