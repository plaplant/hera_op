#! /bin/bash
#This version is meant to run on badant files that include antennas with 180 degree phase switches on the ee pol (both pols would eliminate too many antennas). 
set -e
...
# make filename
bad_ants_fn=`echo "${bad_ants_dir}/${jd_int}.noflip_ee.txt"`
exants=$(prep_exants ${bad_ants_fn})
omnivis_ext=.noflip_ee.omni_vis.uvh5
omnical_ext=.noflip_ee.omni.calfits
firstcal_ext=.noflip_ee.first.calfits

echo redcal_run.py ${fn} --ex_ants ${exants} --ant_z_thresh ${ant_z_thresh} --solar_horizon ${solar_horizon} --flag_nchan_low ${flag_nchan_low} --flag_nchan_high ${flag_nchan_high} --nInt_to_load ${nInt_to_load} --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut} --omnical_ext ${omnical_ext} --omnivis_ext ${omnivis_ext}  --firstcal_ext ${firstcal_ext} --clobber --verbose
redcal_run.py ${fn} --ex_ants ${exants}  --ant_z_thresh ${ant_z_thresh} --solar_horizon ${solar_horizon} --flag_nchan_low ${flag_nchan_low} --flag_nchan_high ${flag_nchan_high} --nInt_to_load ${nInt_to_load} --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut} --omnical_ext ${omnical_ext} --omnivis_ext ${omnivis_ext} --firstcal_ext ${firstcal_ext} --clobber --verbose
